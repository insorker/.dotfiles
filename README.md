...
在配置新安装的系统前，需要改建、换源、配置 git 和安装 zsh。

### 改建

打开 `/usr/share/X11/xkb/keycodes/evdev` 文件，找到 `<CAPS> = 66;` 和 `<RCTL>=105;` 这两行（你系统上的文件的值可能不一样）。

然后把这两个值对换，变成 `<CAPS> = 105;` 和 `<RCTL>=66;` 。然后注销重新登录就生效了。
